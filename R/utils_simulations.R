  ...
  for (k in 1:K)
  {
    ...
    if (dim(t)[1] != 2) {
      ...
      if (sum(prob > 0.5) == 0) {
        ...
        print("all edges are included in the model")
...
summary_mGHS3 = function(res_mGHS, M_true, c_list, ta_est) {
  p = nrow(res_mGHS$Omega_jj)
  p2 = p * (p-1) / 2
  K = length(c_list)  
  ...
  return(list(res = out, ta_reasonable = ta_reasonable))
}
