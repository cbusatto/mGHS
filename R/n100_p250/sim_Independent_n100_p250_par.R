...
cpar = foreach (is = 1:N_iter, .combine = rbind, .packages = c("Rcpp", "mvtnorm", "pROC", "mGHS", "GHS", "JGL", "StatPerMeCo")) %dopar% {
  ...
  res_mGHS = mGHS(B, bn, n, S, p, hyp_ta, 0, 1500)
...
stuck_index = sum(cpar[, 169])

save(stuck_index, res_Independent_mean, res_Independent_g1_mean, res_Independent_g2_mean, res_Independent_g3_mean, res_Independent_g4_mean, file = "sim_n100_p250_mean_Independent.RData")
save(res_Independent_sd, res_Independent_g1_sd, res_Independent_g2_sd, res_Independent_g3_sd, res_Independent_g4_sd, file = "sim_n100_p250_sd_Independent.RData")
