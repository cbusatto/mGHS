...
Rcpp::List mGHS_pred (const int& B, const int& bn, const Eigen::VectorXd& n, const Rcpp::List& S, const int& p, const Rcpp::List& X_test, const int& j_train, const Eigen::VectorXi& n_test, const Eigen::VectorXd hyp_ta, const int& ping = 2500) 
{
  ...
  const int i2 = p - j_train;
  Eigen::MatrixXd y_p(i2, n_test(0)); y_p.setZero();
  Eigen::MatrixXd Xt(n_test(0), j_train); Xt.setZero();
  Eigen::MatrixXd S11_inv(j_train, j_train); S11_inv.setZero();
  Eigen::MatrixXd S12(i2, j_train); S12.setZero();
  ...
  for (h = 0; h < K; h++)
  {
    ...
    y_p.resize(i2, n_test(h)); y_p.setZero();
    y_pred[h] = y_p;
  }
  ...
  for (b = 0; b < B; b++)
  {
    ...
    for (h = 0; h < K; h++)
    {
      ...
      for (j = 0; j < p-1; j++)
      {
        for (js = j+1; js < p; js++)
        {
          ...
          qta2(cs) = 1.0 - pgamma3p1(std::sqrt((1.0 - ta_s) / ta_s), alpha_l, beta_l);
          if (Z(cs, h) == 1)
          {
            l2 += std::log(qta2(cs));
          } else 
    ...
    if (b >= bn)
    {
      ...
      for (h = 0; h < K; h++)
      {
        ...
        for (j = 0; j < j_train; j++)
        {
          for (js = 0; js < j_train; js++)
            S11_inv(js, j) = Sigma[js][j][h];
          
          for (js = j_train; js < p; js++)
            S12(js-j_train, j) = Sigma[js][j][h];
        }
        
        S11_inv = S11_inv.inverse();
        
        // Rcpp::Rcout << "\n S12: \n" << S12;
        // Rcpp::Rcout << "\n S11_inv: \n" << S11_inv;
        
        y_pred[h] = y_p + S12 * S11_inv * Xt.leftCols(j_train).transpose();
      }
    }
  ...
  return Rcpp::List::create(Rcpp::Named("Omega_ij") = Omega_ij_out / (B - bn), Rcpp::Named("Omega_jj") = Omega_jj_out / (B - bn), Rcpp::Named("ta") = ta_out / (B - bn), 
                            Rcpp::Named("y_pred") = y_pred, Rcpp::Named("l1_chain") = l1_out, Rcpp::Named("lambda_prob") = lambda_prob / (B - bn), 
                            Rcpp::Named("eta") = eta_out / (B - bn), Rcpp::Named("R") = R_out / (B - bn), Rcpp::Named("tau") = tau_out / (B - bn));
}
// end file
